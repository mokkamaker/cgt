...
def process(dataset, location):
    benchmarks = {}
    for fn in glob.glob(os.path.join(location,"Zeus*.csv")):
    ...
    for bid,result in benchmarks.items():
        if m:= ADDRESS.search(bid):
            bm.unify.save(dataset, bid, sorted(result), addr=m[0], addr_is_orig=False)
        else:
            bm.unify.save(dataset, bid, sorted(result))
