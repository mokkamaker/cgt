...
def process(dataset, location):
    benchmarks = {}
    onchain = bm.io.read_lines(os.path.join(location,"manual_onchain_src.txt"))
    for line in onchain:
        ...
        if a in benchmarks:
            ...
            benchmarks[a] = 1
            bid = a
        bm.unify.save(dataset, bid, [("EO onchain",s)],
            addr = a, addr_is_orig = addr_is_orig)

    offchain = bm.io.read_lines(os.path.join(location,"manual_offchain_src.txt"))
    del s
    for line in offchain:
        ...
        benchmarks[a] = 1
        bm.unify.save(dataset, bid, [("EO offchain",s)], addr=a)
