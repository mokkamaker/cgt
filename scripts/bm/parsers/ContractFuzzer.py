...
def process(dataset, location):
    benchmarks = set()
    for fn_bin in glob.glob(os.path.join(location,"examples","*","verified_contract_bins","*")):
        ...
        for sol_base in (name,f"{name}.sol"):
            fn_sol = os.path.join(location,"examples",vuln,"verified_contract_sols",sol_base)
        ...
        bm.unify.save(dataset, bid, [(vuln,True)],
            sol = sol,
            bytecode = bytecode,
            runtime = runtime,
            etc = etc)
