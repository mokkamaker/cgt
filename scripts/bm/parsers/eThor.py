...
def process(dataset, location):
    contracts = {}
    for jfn in glob.glob(os.path.join(location,"contract*","*.json")):
    ...
    results = bm.io.read_csv(os.path.join(location, "results.csv"), delimiter=";")
    benchmarks = set()
    for r in results:
        ...
        bm.unify.save(dataset, bid, [("reentrancy", status(r["ground-truth"]))],
            addr=bid,
