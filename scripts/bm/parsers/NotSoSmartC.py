...
def process(dataset, location):
    ...
    for row in bm.io.read_csv(fn_csv, delimiter=";"):
        ...
        for s in row["source"].split(","):
            src.extend(bm.io.read_lines(os.path.join(location,*s.split("/"))))
        ...
        info = [ os.path.join(location, *row["info"].split("/")) ]
        if row["exploit"]:
            info.append(os.path.join(location, *row["exploit"].split("/")))

        bm.unify.save(dataset, bid, classification,
            sol = fn_sol,
