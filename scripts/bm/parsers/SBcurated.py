...
def process(dataset, location):
    benchmarks = {}
    vulnerabilities = bm.io.read_json(os.path.join(location, "vulnerabilities.json"))
    for contract in vulnerabilities:
        ...
        if bid in benchmarks:
            ...
            benchmarks[bid] = 1
        sol = os.path.join(location,os.path.join(*contract["path"].split('/')))
        ...
        bm.unify.save(dataset, bid, sorted(classification),
