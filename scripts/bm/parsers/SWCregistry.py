...
def do_classifications(dataset, bid, jdata, ydata, solpath):
    hash2contract = {}
    contracts = []
    for k,v in jdata['contracts'].items():
    ...
    for contract in flagged:
        ...
        bm.unify.save(dataset, bid, [(vulnerability,status)],
            sol=solpath,
...
def process(dataset, location):
    benchmarks = set()    
    for sfn in glob.glob("**/*.sol", root_dir=location, recursive=True):
        asfn = os.path.join(location,sfn)
        ...
        try:
            do_classifications(dataset, bid, jdata, ydata, asfn)
